...
public class RegthermMeteotestDataWorkspace {
	private static final String TAG = "Regtherm xml Serializer";

	private static RegthermMeteotestDataWorkspace instance;

	public static RegthermMeteotestDataWorkspace getInstance() {
		if(instance == null)
		{
			instance = new RegthermMeteotestDataWorkspace();
		}
	...
	private RegthermData regthermData;

	public RegthermMeteotestDataWorkspace initialize(String applicationNamen,
		...
		this.pageId = downloadUrl + " " + postArguments;
		return this;
	}

	public RegthermData loadRegthermData() {
		if(this.regthermData == null){
//			this.regthermData = readFile();
			this.regthermData = new RegthermData();
			
		}
		
		this.downloadFileFromUrl(regthermData);
		
//		if (regthermData == null) {
//			// download file
//			regthermData = new RegthermData();
//			this.downloadFileFromUrl(regthermData);
//		} else {
//			RegthermPageData page = regthermData.findPageData(this.pageId);
//			if (page == null) {
//				this.downloadFileFromUrl(regthermData);
//			}
//		}
		
		this.asyncRefreshRegthermData(regthermData);
	...
	public static String getPath() {
		return Environment.getExternalStorageDirectory() + File.separator
				+ applicationName + File.separator + "RegthermDataFlugbasis.xml";
	}
	...
	private static synchronized void initializeHttpClient() {
		...
		if(client == null){
			...
			sHttpParams.setParameter("http.socket.timeout", 2000);
			sHttpParams.setParameter("http.connection.timeout", 2000);
	...
	private synchronized void downloadFileFromUrl(RegthermData regthermData) {
		...
		if(this.jSonString == null || this.jSonString.equals("")){
			this.readJsonStringFromPage();	
		}

		RegthermPageData pageData = regthermData.findPageData(pageId);

		if (pageData == null) {
			pageData = new RegthermPageData();
			pageData.setPageUrl(pageId);
			regthermData.regthermPages.add(pageData);
		}

		parseJsonString(jSonString, pageData);

//		persistRegthermData(regthermData);

	}

	private void readJsonStringFromPage() {
		try {
			initializeHttpClient();
			
			HttpGet request = new HttpGet(downloadUrl);
			finalHttpResponse = client.execute(request);
			InputStream in = finalHttpResponse.getEntity().getContent();
	...
	private void parseJsonString(String jsonString, RegthermPageData pageData) {

		try {
			pageData.resetRows();
			JSONObject jObject = new JSONObject(jsonString);
			JSONObject regThermPage = jObject.getJSONObject(postArguments);

			pageData.headerRow = regThermPage.getString("title");

			String rawData = regThermPage.getString("rawshort");
			
			String basis = regThermPage.getJSONObject("max").getString("basis");
			String top = regThermPage.getJSONObject("max").getString("top");
			double maxWindKmh = Math.round(regThermPage.getJSONObject("max").getLong("ff"));
			String maxTherm = regThermPage.getJSONObject("max").getString("thermal");
			
			pageData.basis = basis == "null" ?  "" : basis;
			pageData.top = top == "null" ?  "" : top;
			pageData.maxtherm = maxTherm == "null" ?  "" : maxTherm;
			pageData.maxwindkmh = maxWindKmh;
